...
export default function RegistrationTable({
    ...
    if (confirm("Are you sure you want to set this registration to pending?")) {
      startTransition(async () => {
        try {
          ...
          if (!result.success) {
            setTimeout(
              () => alert(result.error || "Failed to change status to pending"),
              0,
            );
          }
        } catch (error) {
  ...
  const handleDelete = (id: string) => {
    if (
      confirm(
        "Yakin ingin menghapus registrasi ini? Tindakan ini tidak dapat dibatalkan.",
      )
    ) {
      startTransition(async () => {
        try {
          ...
          <div className="bg-gradient-to-b from-[#390D62] to-[#1a062d] border-3 border-[#AAF3D5] rounded-2xl max-w-2xl w-full p-6 text-white relative shadow-2xl overflow-y-auto max-h-[80vh]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1">
                <p className="text-[#AAF3D5]/70 text-xs uppercase tracking-wider font-bold">
                  Competition
                </p>
                <p className="text-xl font-semibold">
                  {selectedReg.competition.name}
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-[#AAF3D5]/70 text-xs uppercase tracking-wider font-bold">
                ...
                <div>
                  <span
                    ...
                    ${
                      selectedReg.registration_status === "Registered"
              ...
              <div className="space-y-1">
                ...
                <p className="text-lg font-semibold">
                  {selectedReg.user.faculty || "Not Set"}
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-[#AAF3D5]/70 text-xs uppercase tracking-wider font-bold">
        ...
        <table className="min-w-full bg-black/30 shadow-sm rounded-lg border-[#AAF3D5] border-3">
          <thead className="border-3 border-[#AAF3D5] overflow-hidden rounded-lg">
            <tr className="rounded-2xl">
              <th className="px-6 py-3 text-left bg-gradient-to-b from-[#390D62] to-[#6226A4] text-xs font-medium text-white uppercase tracking-wider border-[#AAF3D5] border-3">
                No
              </th>
              <th className="px-6 py-3 text-left bg-gradient-to-b from-[#390D62] to-[#6226A4] text-xs font-medium text-white uppercase tracking-wider border-[#AAF3D5] border-3">
                Competition
            ...
            {paginatedRegistrations.length > 0 ? (
              paginatedRegistrations.map((registration, index) => (
                <tr key={registration.id}>
                  ...
                  <td className="px-6 py-4 bg-black/30 border-r whitespace-nowrap text-sm font-medium">
                  ...
                  <td className="bg-black/30 flex gap-4 px-6 py-4 whitespace-nowrap text-sm">
                    ...
                    <button
                      className="bg-black/30 hover:bg-purple-300/25 border-white border-3 hover:cursor-pointer px-4 py-2 rounded-lg"
                      onClick={() => handleSetPending(registration.id)}
                    >
                      Pending
                    </button>
                    <button
                    ...
                    <button
                      className="bg-red-700/60 hover:bg-red-600/80 border-red-400 border-3 hover:cursor-pointer px-4 py-2 rounded-lg text-white font-semibold"
