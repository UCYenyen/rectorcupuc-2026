...
export default function VoteCard({
  ...
  const handleUnvote = async () => {
    ...
    if (!confirm(`Are you sure you want to remove your vote for ${name}?`))
      return;
  ...
  return (
    <PixelCard
      variant="pink"
      className="border-3 border-white rounded-lg relative h-[320px] max-w-[275px] flex items-stretch"
    >
      <div className="absolute inset-0 bg-black/40 backdrop-blur-2xl flex flex-col items-center w-full justify-between p-4 gap-2 overflow-hidden">
        <h2 className="text-white text-xl font-bold text-center line-clamp-2 w-full flex-shrink-0">
          {name}
        </h2>
