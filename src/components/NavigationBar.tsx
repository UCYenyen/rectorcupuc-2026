...
export default function NavigationBar() {
  ...
  // Configuration (diambil dari StaggeredMenu sebelumnya)
  const colors = ['#6427A6', '#E979EE', '#ADDCE7'];
  const accentColor = '#E979EE';

  // --- LOGIC ITEMS (Exist di Navibar) ---
  const menuItems = [
    { label: "Home", href: "/" },
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 z-[210] bg-gradient-to-r from-[#6427A6] to-[#E979EE] flex min-h-[7vh] py-2 px-[5%] justify-between items-center w-screen overflow-visible shadow-2xl border-y-4 border-[#ADDCE7]">
        <StripeBackground />
        ...
        <div className="md:hidden flex items-center gap-4 relative z-10">
          ...
          <button 
            onClick={toggleMenu}
            className="flex items-center justify-center bg-black/40 backdrop-blur-md border-2 border-white rounded-lg p-2 text-white transition-all active:scale-90"
          >
      ...
      {/* --- STAGGERED LAYERS & ASIDE (Mobile Only) --- */}
      <div ref={preLayersRef} className="fixed inset-0 z-[190] pointer-events-none md:hidden">
        {colors.map((c, i) => (
