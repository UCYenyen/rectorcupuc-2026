...
export async function createTeam(
  ...
  const isTeamExist = await prisma.team.findUnique({
    where: {
      name_competition_id: { name: name, competition_id: competitionId },
    },
  });

  if (isTeamExist) {
    return {
      error: "Team with this name is already registered in the competition.",
    };
  }
  ...
  try {
    ...
    return {
      error: "Leader ID tidak ditemukan atau terjadi kesalahan database.",
    };
  }
}
...
export async function checkIsAlreadyMember(userId: string, teamId: string) : Promise<TeamMember | null> {
  return await prisma.teamMember.findFirst({
    where: {
      user_id: userId,
      team_id: teamId,
      NOT: { join_request_status: "Failed" },
    },
  });
}

export async function joinTeamByReferalCode(
  userId: string,
  referalCode: string,
  ...
  if (leader.faculty !== faculty) {
    ...
    if (profileUrl) await deleteUploadedImage(profileUrl);
    return {
      error: `Faculty mismatch! This team is for ${leader.faculty} students only. You selected ${faculty}.`,
    };
  }
  ...
  if (competitionRegistration.registration_status != "Registered") {
    ...
    if (profileUrl) await deleteUploadedImage(profileUrl);
    return {
      error: `Team registration status for ${team.name} is ${competitionRegistration.registration_status}! Please wait until the approval is granted.`,
    };
  }
  ...
  const isAlreadyMember = await checkIsAlreadyMember(userId, team.id);
  ...
  if (alreadyRegisteredToAnotherTeam) {
    ...
    if (profileUrl) await deleteUploadedImage(profileUrl);
    return {
      error: "You are already registered to another team for this competition!",
    };
  }
  ...
  try {
    ...
    const previousFailedRequest = await prisma.teamMember.findFirst({
      where: {
        user_id: userId,
        team_id: team.id,
    ...
    if (previousFailedRequest) {
      teamMember = await prisma.teamMember.update({
        where: { id: previousFailedRequest.id },
        data: {
