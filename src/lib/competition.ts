...
export async function getCompetitionBySlug(
  slug: string,
): Promise<CompetitionContainerProps | null> {
...
export async function registerTeamToCompetition(
  ...
  const isTeamExist = await prisma.team.findUnique({
    where: {
      name_competition_id: { name: teamName, competition_id: competition.id },
    },
  });
  if (isTeamExist)
    return { error: "Team with this name is already registered." };
...
export async function createMatch(
  prevState: CreateMatchFormState,
  formData: FormData,
